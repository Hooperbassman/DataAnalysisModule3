...
select * from orders order by order_datetime desc limit 5;
...
select orders.order_id, orders.status, COUNT(order_items.order_item_id) as total_items from orders join order_items on orders.order_id = order_items.order_id group by orders.order_id, orders.status;
